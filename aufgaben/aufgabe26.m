...
Prototyp_Schraube_z = 1/8 * sum(D_strich(1:8,3)); %row 3
...
figure(3) 
plot3(D_strich(:,1),D_strich(:,2),D_strich(:,3),'b.') %plots a three dimensional plot of the column vectors D_strich(:,1), D_strich(:,2), D_strich(:,3)
hold on 
plot3(Prototyp_Schraube_x,Prototyp_Schraube_y,Prototyp_Schraube_z,'c.')
plot3(Prototyp_Muttern_x,Prototyp_Muttern_y,Prototyp_Muttern_z,'m.')
...
figure(4)
plot3(Prototyp_Schraube_x,Prototyp_Schraube_y,Prototyp_Schraube_z,'kx') %three dimensional feature point with coordinates Prototyp_Schraube_x,Prototyp_Schraube_y,Prototyp_Schraube_z
hold on
plot3(Prototyp_Muttern_x,Prototyp_Muttern_y,Prototyp_Muttern_z,'ko') %three dimensional feature point with coordinates in x, y, and z direction
...
for i=1:14
    ...
    if Abstand_Schraube <= Abstand_Mutter
        Gruppe = 'Schraube';
        plot3(D_strich(i,1),D_strich(i,2),D_strich(i,3),'c.')
        fprintf('Dateiname:%s \t Gruppe:%s \n',Bilder(i+2).name,Gruppe);
    else
        Gruppe = 'Mutter';  
        plot3(D_strich(i,1),D_strich(i,2),D_strich(i,3),'m.')
        fprintf('Dateiname:%s \t Gruppe:%s \n',Bilder(i+2).name,Gruppe);
    end 
end 
